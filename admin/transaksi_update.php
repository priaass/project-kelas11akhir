...
if (!$row_paket) {
    echo "<script>alert('Paket tidak valid!'); window.location.href='transaksi_edit.php?id=$id';</script>";
    exit();
}
...
$query_update = "UPDATE tr_cuci_kering SET nama='$username', no_hp='$no_hp', alamat='$alamat', paket_id='$paket_id', waktu='$waktu', berat='$berat', tarif='$tarif', tgl_keluar='$tgl_keluar', total_bayar='$total_bayar', status='$status' WHERE id='$id'";
